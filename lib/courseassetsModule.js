  ...
  */
  ...
  async insertContent(results) {
    ...
    const finalCourseAssets = fileArray.reduce((promiseChain, assetItem) =>
      promiseChain.then(() => this.handleSingleAsset('insert', assetItem, courseId, contentId)), Promise.resolve());
  ...
  async deleteContent(results) {
    ...
    const finalCourseAssets = fileArray.reduce((promiseChain, assetItem) =>
      promiseChain.then(() => this.handleSingleAsset('delete', assetItem, courseId, contentId)), Promise.resolve());
  ...
  async updateContent(originalDoc, results) {
    ...
    const deleteOriginalAssets = async (origDoc) => {
      return await this.deleteContent(origDoc);
    }

    return deleteOriginalAssets(originalDoc).then(await this.insertContent(results));

  }

  /**
  * Search for asset and action
  * @param {String} action
  * @param {Object} assetName
  * @param {String} courseId
  * @param {String} contentId
  */
  async handleSingleAsset(action, assetName, courseId, contentId) {
