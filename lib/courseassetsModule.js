...
class CourseAssetsModule extends AbstractApiModule {
  ...
  */
  async init() {
    ...
    */
    ...
    this.app.onReady().then(async () => {
      ...
      // TODO deal with content replace or update
  ...
  async insert(data, options, mongoOptions) {
    ...
    if (typeof data !== 'object') throw new Error(this.app.lang.t('error.emptyfields'));
    if (!data._courseId) throw new Error(this.app.lang.t('error.courseId'));
  ...
  async delete(data, options, mongoOptions) {
    if (typeof data !== 'object') throw new Error(this.app.lang.t('error.emptyfields'));
    if (!data._id) throw new Error(this.app.lang.t('error.emptyfields'));

    try {
      const existingRecord = await this.find({ _id: data._id });

      if (!existingRecord || existingRecord.length === 0) throw new Error(this.app.lang.t('error.deleteerror'));

      if (existingRecord[0].assetCount === 1) {
        return await super.delete({ _id: data._id });
      }
  ...
  async insertContent(results) {
    ...
    if (!results._courseId) throw new Error(this.app.lang.t('error.courseId'));
    ...
    // TODO seperate this out into chainable async functions. So it can be used by deleteContent
    const checkAssets = async (assetId) => {

      await this.assets.find({ path: assetId })
        .then((assetRecords) => {
    ...
    const finalCourseAssets = fileArray.reduce((promiseChain, arrayItem) =>
      promiseChain.then(() => checkAssets(arrayItem)), Promise.resolve());
  ...
  async deleteContent(results) {
    ...
    if (results._type === 'course') {
      return await super.delete({ _courseId: results._courseId });
    }
